...
void init_ledc() {
    ...
    if (err != ESP_OK) {
        log_mqtt(LOG_ERROR, TAG, true, "Error (%s) setting channel on pin %d", esp_err_to_name(err), LEDC_LS_CH2_GPIO);
        return;
    }
    ...
    err = ledc_cb_register(ledc_channel.speed_mode, ledc_channel.channel, &callbacks, (void *) counting_sem);
    if (err != ESP_OK) {
        log_mqtt(LOG_ERROR, TAG, true, "Error (%s) registering callback", esp_err_to_name(err));
        return;
    }
...
void ledc_duty_fade(const uint32_t duty) {
    ...
    err = ledc_set_fade_with_time(ledc_channel.speed_mode,
                                    ledc_channel.channel, duty, LEDC_TEST_FADE_TIME);
    if (err != ESP_OK) {
        log_mqtt(LOG_ERROR, TAG, true, "Error (%s) setting fade duty %d pin %d",
            esp_err_to_name(err), duty, LEDC_LS_CH2_GPIO);
        return;
    }

    err = ledc_fade_start(ledc_channel.speed_mode,
                    ledc_channel.channel, LEDC_FADE_NO_WAIT);
    if (err != ESP_OK) {
        log_mqtt(LOG_ERROR, TAG, true, "Error (%s) starting fade duty %d pin %d",
            esp_err_to_name(err), duty, LEDC_LS_CH2_GPIO);
        return;
    }

    log_mqtt(LOG_INFO, TAG, true, "Starting fade : %d, on pin %d", duty, LEDC_LS_CH2_GPIO);
...
void ledc_duty(const uint32_t duty) {
    ...
    err = ledc_set_duty(ledc_channel.speed_mode, ledc_channel.channel, duty);
    if (err != ESP_OK) {
    ...
    log_mqtt(LOG_INFO, TAG, true, "Duty : %d, on pin %d", duty, LEDC_LS_CH2_GPIO);
}
