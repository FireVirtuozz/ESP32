...
static volatile bool front_blocked = false;
static volatile bool rear_blocked = false;

esp_err_t get_rear_blocked(bool* blocked) {
    #if CONFIG_USE_HCSR04
    *blocked = rear_blocked;
    return ESP_OK;
    #else
...
static double last_distance_front = 0.0;
static double last_distance_rear = 0.0;
...
static void hc_task(void * params) {
    ...
    while (monitoring)
    {
        ...
        if (val_hc >= 0 && val_hc < 30000) {
            ...
            int16_t motor = 0;
            bool last_sign_positive = false;
            bool blocking = false;

            if (get_active_hc_blocking(&blocking) == ESP_OK && blocking) {
                if (get_motor_percent(&motor) == ESP_OK && get_last_motor_sign_positive(&last_sign_positive) == ESP_OK) {
                    if (hc_cfg->hc_id == 0) { // front hc — plus de "&& last_sign_positive" ici
                        // le déclenchement reste conditionné au sens
                        if (((val_hc / 58.0 - last_distance_front < -2.0 && val_hc / 58.0 < motor * 0.1 && last_sign_positive)
                            || val_hc / 58.0 < 10.0) && !front_blocked) {
                            force_motor_stop();
                            front_blocked = true;
                            log_msg_lvl(ESP_LOG_WARN, TAG, "Front HC blocked: val: %.2f", val_hc / 58.0);
                        }
                        // déblocage : indépendant du sens actuel, s'exécute toujours pour hc_id == 0
                        if (front_blocked && val_hc / 58.0 > 10.0) {
                            log_msg_lvl(ESP_LOG_WARN, TAG, "Front HC unblocked: val: %.2f", val_hc / 58.0);
                            front_blocked = false;
                        }
                        last_distance_front = val_hc / 58.0;
                    }
                    if (hc_cfg->hc_id == 1) { //rear hc // le déclenchement reste conditionné au sens
                        if (((val_hc / 58.0 - last_distance_rear < -2.0 && val_hc / 58.0 < -motor * 0.1 && !last_sign_positive)
                            || val_hc / 58.0 < 10.0) && !rear_blocked) {
                            force_motor_stop();
                            rear_blocked = true;
                            log_msg_lvl(ESP_LOG_WARN, TAG, "Rear HC blocked: val: %.2f", val_hc / 58.0);
                        }
                        // déblocage : indépendant du sens actuel, s'exécute toujours pour hc_id == 0
                        if (rear_blocked && val_hc / 58.0 > 10.0) {
                            log_msg_lvl(ESP_LOG_WARN, TAG, "Rear HC unblocked: val: %.2f", val_hc / 58.0);
                            rear_blocked = false;
                        }
                        last_distance_rear = val_hc / 58.0;
                    }
                }
            }
            ...
            uint8_t buf[HEADER_SENSOR_SIZE + HC_PAYLOAD_SIZE + 2];
            serialize_header(&header, buf);
            buf[HEADER_SENSOR_SIZE] = hc_cfg->hc_id;
            memcpy(&buf[HEADER_SENSOR_SIZE + 1], &val_hc, sizeof(int64_t));
            buf[HEADER_SENSOR_SIZE + 1 + HC_PAYLOAD_SIZE] = (uint8_t)(hc_cfg->hc_id == 0 ? front_blocked : rear_blocked);
            //print_hc(val_hc);
...
static esp_err_t get_ina_info(ina_info_t *ina_info) {
    if (ina_info == NULL) {
...
#define BMP_PAYLOAD_SIZE (2 * sizeof(int32_t))

typedef struct bmp_info_st {
    int32_t temperature;
    int32_t pressure;
} bmp_info_t;
...
static void print_bmp(bmp_info_t *bmp) {
    log_msg(TAG, "Temperature BMP received: %d, value: %.2f°C", bmp->temperature,
        bmp->temperature / 100.0);
    log_msg(TAG, "Pressure received: %d, value: %.3fbar", bmp->pressure, bmp->pressure * 1e-5);
}

/**
 * Initialize BMP280 sensor i2c
 */
static esp_err_t init_bmp() {
    esp_err_t err;
    ...
    log_msg(TAG, "BMP280 initialized");
 ...
 */
static esp_err_t get_bmp_info(bmp_info_t *bmp_info) {
    if (bmp_info == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    bmp_info->pressure = (buf_bmp[0] << 12) | (buf_bmp[1] << 4) | (buf_bmp[2] >> 4); //20 bits word
    bmp_info->temperature = (buf_bmp[3] << 12) | (buf_bmp[4] << 4) | (buf_bmp[5] >> 4);

    err = convert_temperature(&bmp_info->temperature);
    if (err != ESP_OK) return err;

    err = convert_pressure(&bmp_info->pressure);
    if (err != ESP_OK) return err;
...
static void serialize_bmp(bmp_info_t *bmp, uint8_t *buf) {
    uint16_t len = HEADER_SENSOR_SIZE;
    memcpy(&buf[len] ,&bmp->pressure, sizeof(int32_t));
    len += sizeof(int32_t);
    memcpy(&buf[len] ,&bmp->temperature, sizeof(int32_t));
}

/**
 * 50 ms monitoring BMP task
 */
static void bmp_task(void * params) {
    esp_err_t err;
    bmp_info_t bmp_info;
    ...
    while (monitoring)
    {
        ...
        if (err == ESP_OK) {
            ...
            uint8_t buf[HEADER_SENSOR_SIZE + BMP_PAYLOAD_SIZE];
            serialize_header(&header, buf);
            serialize_bmp(&bmp_info, buf);
            //print_bmp(&bmp_info);
...
static esp_err_t init_mpu() {
    ...
    uint8_t pwr_wakeup[] = {0x6B, 0x01}; 
    err = i2c_master_transmit(mpu_handle, pwr_wakeup, sizeof(pwr_wakeup), -1);
    if (err != ESP_OK) {
        log_msg_lvl(ESP_LOG_ERROR, TAG, "Error (%s) waking up mpu", esp_err_to_name(err));
        return err;
    }
 ...
 * @returns ESP_OK success, others error 
 */
static esp_err_t get_mpu_info(mpu_info_t *mpu_info) {
    if (mpu_info == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
...
#define MPU_PAYLOAD_SIZE (7 * sizeof(int16_t))
...
static void rcwl_task(void * params) {
    ...
    while (monitoring)
    {
        if (xSemaphoreTake(sem_rcwl, portMAX_DELAY) == pdTRUE) {
            ...
            log_msg(TAG, "rcwl edge value : %d", detected_motion);
...
static volatile uint16_t pulses_count_20ms = 0;
static volatile uint16_t pulses_count_100ms = 0;
...
static uint16_t pulse_history[SLIDING_WINDOW_SIZE] = {0};
...
static void ky033_task(void * params) {
    ...
    while (monitoring)
    {
        ...
        pulses_count_20ms = (uint16_t)pcnt_count;
        total_pulses += pulses_count_20ms;

        pulse_history[history_idx] = pulses_count_20ms;
        history_idx = (history_idx + 1) % SLIDING_WINDOW_SIZE;

        uint16_t current_100ms_sum = 0;
        for (uint8_t i = 0; i < SLIDING_WINDOW_SIZE; i++) {
            current_100ms_sum += pulse_history[i];
        }
        pulses_count_100ms = current_100ms_sum;
        ...
        if (reset_count == 5) {
            ...
            serialize_header(&header, buf);
            buf[HEADER_SENSOR_SIZE] = (uint8_t)pulses_count_100ms;
            int16_t motor = -1001;
            ...
            err = get_last_motor_sign_positive(&motor_sign_positive);
            if (err != ESP_OK) {
                log_msg_lvl(ESP_LOG_ERROR, TAG, "Error (%s) reading MOTOR", esp_err_to_name(err));
            }
        #endif
        ...
        #if CONFIG_USE_ESPNOW
        #endif
            log_msg_lvl(ESP_LOG_DEBUG, TAG, "Send udp ky033, 100ms: %u, 20ms: %u, reset_count: %u",
                pulses_count_100ms, pulses_count_20ms, reset_count
            );
            reset_count = 0;
        }
...
static esp_err_t init_dht11() {
    ...
    // 1. Configuration permanente du GPIO en Open-Drain Input/Output avec Pull-Up
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << OUT_PIN_DHT11),
        .mode = GPIO_MODE_INPUT_OUTPUT_OD,
        .pull_up_en = GPIO_PULLUP_ENABLE, // Pull-up interne activé (externe recommandé de 4.7k)
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    };
    gpio_config(&io_conf);
    gpio_set_level(OUT_PIN_DHT11, 1); // Ligne au repos à '1'

    // 2. Création du sémaphore
    ...
    // 3. Configuration RMT RX
    rmt_rx_channel_config_t rx_config = {
        .gpio_num = OUT_PIN_DHT11,
        .clk_src = RMT_CLK_SRC_DEFAULT,
        .resolution_hz = 1000000, // 1 us / tick
...
static void dht11_task(void * params) {
    ...
    while (monitoring)
    {
        // 1. SIGNAL START (Maintien à '0' pendant 20ms)
        gpio_set_level(OUT_PIN_DHT11, 0);
        vTaskDelay(pdMS_TO_TICKS(20));

        // 2. RELÂCHEMENT À '1' + PAUSE DE 25µs (Spécification DHT11)
        gpio_set_level(OUT_PIN_DHT11, 1);
        esp_rom_delay_us(25); // Laisse la ligne remonter avant que le DHT11 ne réponde

        // 3. ARMEMENT DU RMT JUSTE AU BON MOMENT
        rmt_receive_config_t receive_config = {
            .signal_range_min_ns = 1000,   // Filtre les parasites < 1us
            .signal_range_max_ns = 150000, // Timeout si la ligne reste inactive 150us
        };
        ...
        if (xSemaphoreTake(dht11_rx_sem, pdMS_TO_TICKS(100)) == pdTRUE) 
        {
            ...
            // RECHERCHE DYNAMIQUE DU HANDSHAKE (Signal bas du DHT11 entre 60us et 100us)
            int start_index = -1;
            for (int i = 0; i < 5; i++) {
                uint32_t dur0 = dht11_rx_buffer[i].duration0;
                uint32_t dur1 = dht11_rx_buffer[i].duration1;

                // Le handshake du DHT11 est un niveau BAS (~80us) suivi d'un niveau HAUT (~80us)
                if ((dur0 >= 50 && dur0 <= 110) && (dur1 >= 50 && dur1 <= 110)) {
                    start_index = i + 1; // Les 40 bits de données commencent juste après
                    break;
                }
            }

            if (start_index != -1) {
                // DÉCODAGE DES 40 BITS
                for (int i = 0; i < 40; i++) {
                    rmt_symbol_word_t sym = dht11_rx_buffer[start_index + i];
                    
                    uint32_t low_dur  = (sym.level0 == 0) ? sym.duration0 : sym.duration1;
                    uint32_t high_dur = (sym.level0 == 1) ? sym.duration0 : sym.duration1;

                    // Un bit '1' a une durée HAUTE (>40us) supérieure à la durée BASSE (~50us)
                    if (high_dur > low_dur) {
                        data[i / 8] |= (1 << (7 - (i % 8)));
                    }
                }
                ...
                if (data[4] == ((data[0] + data[1] + data[2] + data[3]) & 0xFF)) 
                {
                    ...
                    //log_msg(TAG, "DHT11 humidity: %u%%, temp: %u°C", humidity, temperature);
                    ...
                    log_msg(TAG, "Checksum failed | Raw: H=%d%% T=%d°C | Calc: %d vs Recv: %d", 
                            data[0], data[2], 
                            ((data[0] + data[1] + data[2] + data[3]) & 0xFF), data[4]);
                }
            } else {
                // LOG DE DEBUG : Affiche les 3 premiers symboles reçus si le Handshake n'est pas trouvé
                log_msg(TAG, "Handshake not found! Sym0: L0=%d D0=%ld, L1=%d D1=%ld | Sym1: L0=%d D0=%ld, L1=%d D1=%ld",
                        dht11_rx_buffer[0].level0, dht11_rx_buffer[0].duration0,
                        dht11_rx_buffer[0].level1, dht11_rx_buffer[0].duration1,
                        dht11_rx_buffer[1].level0, dht11_rx_buffer[1].duration0,
                        dht11_rx_buffer[1].level1, dht11_rx_buffer[1].duration1);
            }
        } 
        else {
...
#define OUT_PIN_DS18B20 23 // Pin GPIO de ta sonde

static portMUX_TYPE ds_spinlock = portMUX_INITIALIZER_UNLOCKED;

// --- 1. PRIMITIVES TIMING 1-WIRE MATÉRIEL ---

// Signal de Reset et détection de présence
static bool ds18b20_reset(void) {
    bool presence = false;

    taskENTER_CRITICAL(&ds_spinlock);

    // 1. Reset Pulse : Ligne à 0 pendant 480 µs mesurées à l'horloge
    int64_t start = esp_timer_get_time();
    gpio_set_level(OUT_PIN_DS18B20, 0);
    while ((esp_timer_get_time() - start) < 480);

    // 2. Relâchement de la ligne
    gpio_set_level(OUT_PIN_DS18B20, 1);
    
    // 3. Mesure exacte du temps où la ligne reste à 0 dans les 500 µs suivantes
    start = esp_timer_get_time();
    int64_t low_start = 0;
    int64_t low_duration = 0;

    while ((esp_timer_get_time() - start) < 500) {
        if (gpio_get_level(OUT_PIN_DS18B20) == 0) {
            if (!presence) {
                presence = true;
                low_start = esp_timer_get_time() - start; // Moment où la sonde a répondu
            }
            low_duration++;
        }
    }
    ...
    if (presence) {
        log_msg(TAG, "Sonde détectée ! Réponse après %lld µs (Durée bas: %lld µs)", low_start, low_duration);
    } else {
        log_msg(TAG, "Aucune réponse détectée sur 500 µs");
    }
...
static void ds18b20_write_bit(bool bit) {
    ...
    if (bit) {
        // Ecrire '1' : Impulsion basse très courte (1 à 15µs), puis relâchement
        esp_rom_delay_us(6);
        gpio_set_level(OUT_PIN_DS18B20, 1);
        esp_rom_delay_us(64);
    } else {
        // Ecrire '0' : Maintien au niveau bas pendant 60µs
        esp_rom_delay_us(60);
        gpio_set_level(OUT_PIN_DS18B20, 1);
        esp_rom_delay_us(10);
    }
...
static bool ds18b20_read_bit(void) {
    ...
    taskENTER_CRITICAL(&ds_spinlock);
    gpio_set_level(OUT_PIN_DS18B20, 0);
    esp_rom_delay_us(2); // Impulsion de démarrage du slot (> 1µs)
    
    gpio_set_level(OUT_PIN_DS18B20, 1); // Relâchement de la ligne
    esp_rom_delay_us(10); // Échantillonnage à t = 12µs (doit être fait avant 15µs)
    
    bit = (gpio_get_level(OUT_PIN_DS18B20) != 0);
    taskEXIT_CRITICAL(&ds_spinlock);

    esp_rom_delay_us(50); // Fin du slot de lecture (60µs min)
    return bit;
}

// Écriture d'un octet (LSB First)
...
// Calcul du Checksum CRC8 Maxim/Dallas (Polynôme x^8 + x^5 + x^4 + 1)
...
static esp_err_t init_ds18b20() {
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << OUT_PIN_DS18B20),
        .mode = GPIO_MODE_INPUT_OUTPUT_OD, // Open-Drain I/O
        .pull_up_en = GPIO_PULLUP_ENABLE,  // Pull-up interne
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    };
    gpio_config(&io_conf);
    gpio_set_level(OUT_PIN_DS18B20, 1); // Ligne au repos à 1
    ...
    log_msg(TAG, "DS18B20 1-Wire manual driver initialized");
    return ESP_OK;
}
...
static void ds18b20_task(void * params) {
    ...
    while (monitoring)
    {
        // 1. DEMANDE DE CONVERSION DE TEMPÉRATURE (Convert T)
        if (ds18b20_reset()) 
        {
            ds18b20_write_byte(0xCC); // SKIP ROM (S'adresse à l'unique capteur sur le bus)
            ds18b20_write_byte(0x44); // CONVERT T

            // La conversion 12-bit prend jusqu'à 750ms en interne dans la sonde
            vTaskDelay(pdMS_TO_TICKS(800));

            // 2. LECTURE DU SCRATCHPAD
            if (ds18b20_reset()) 
            {
                ...
                // 3. VÉRIFICATION DU CHECKSUM (CRC8)
                uint8_t calculated_crc = ds18b20_crc8(scratchpad, 8);
                if (calculated_crc == scratchpad[8]) 
                {
                    // Assemblage des 16 bits (LSB = Byte 0, MSB = Byte 1)
                    int16_t raw_temp = (scratchpad[1] << 8) | scratchpad[0];
                    
                    // La résolution 12-bit donne 0.0625°C par LSB
                    float temp_c = raw_temp * 0.0625f;
                    int16_t temp_scaled = (int16_t)(temp_c * 100.0f); // Ex: 24.37°C -> 2437
                    ...
                    log_msg(TAG, "SUCCESS! DS18B20 temp: %.2f°C", temp_c);
                    ...
                    log_msg(TAG, "DS18B20 CRC Mismatch! Calc: 0x%02X vs Recv: 0x%02X", 
        ...
        vTaskDelay(pdMS_TO_TICKS(2000)); // Pause entre 2 mesures
    }
    vTaskDelete(NULL);
}
...
#define KY018_ADC_CHANNEL ADC_CHANNEL_0
...
static void ky018_task(void * params) {
    ...
    while (monitoring) {
        ...
        if (err == ESP_OK) {
            ...
            int32_t val = raw_val;
            //log_msg(TAG, "photoresistor adc raw value: %d", val);
...
#define ESP_PACKET_SIZE (5 * sizeof(uint32_t) + 1 * sizeof(int8_t) + 3 * sizeof(float) + 3 * sizeof(uint8_t) + sizeof(int16_t))
...
static void serialize_esp_packet(esp_packet_t *pkt, uint8_t *buf) {
    ...
    len += sizeof(uint8_t);
    memcpy(&buf[len], &pkt->motor, sizeof(int16_t));
    len += sizeof(int16_t);
...
esp_err_t init_sensors() {
...
#if CONFIG_USE_BMP280
    res = xTaskCreate(bmp_task, "bmp_monitoring", 2048, NULL, 5, NULL);
    if (res != pdPASS) {
        log_msg_lvl(ESP_LOG_ERROR, TAG, "Error (%d) creating bmp monitoring task", res);
        return ESP_FAIL;
    }
#endif
    ...
    res = xTaskCreate(ds18b20_task, "ds18b20_monitoring", 4096, (void *)bus_handle, 5, NULL);
    if (res != pdPASS) {
        log_msg_lvl(ESP_LOG_ERROR, TAG, "Error (%d) creating ds18b20 monitoring task", res);
        return ESP_FAIL;
    }
