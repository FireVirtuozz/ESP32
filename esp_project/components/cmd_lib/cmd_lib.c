...
esp_err_t apply_gamepad_commands(const gamepad_t *gamepad) {
    ...
    if (!last_dpadup && current_dpadup) {
        force_motor_stop();
    }

    if (!last_dpaddown && current_dpaddown) {
        bool current_active = false;
        if (get_active_hc_blocking(&current_active) == ESP_OK) {
            activate_hc_blocking(!current_active);
        }
    }
    ...
    final_speed *= 10;
    log_msg(TAG, "Sending speed target controller: %d", final_speed);

    // 4. Envoi de la commande finale bridée au moteur
    ledc_motor(final_speed);
