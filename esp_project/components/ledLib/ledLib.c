...
static void ledc_duty(const uint32_t duty, const uint8_t idx) {
    ...
    log_msg_lvl(ESP_LOG_WARN, TAG, "Duty : %d, on channel %d", duty, idx);
}
...
static void apply_target_motor(void* args) { 
    ...
    if (atomic_load(&breaking_lock)) {
        ...
        if (esp_timer_get_time() - timestamp_force_motor > 500000) {
            log_msg_lvl(ESP_LOG_WARN, TAG, "Breaking stopped");
            target_motor = 0;
...
#if CONFIG_USE_BTS7960
    if (!atomic_load(&breaking_lock)) {
        log_msg_lvl(ESP_LOG_WARN, TAG, "Breaking started");
