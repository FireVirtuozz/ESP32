...
use crate::{gui::ScreensTypes, ota::{OtaServerStatus, fetch_latest_bin, serve_firmware}};
...
impl OtaScreen {
    pub fn show(&mut self, ctx: &egui::Context, screen: &mut ScreensTypes) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("Firmware :");
                if ui.button("Choisir .bin").clicked() {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.add_enabled(can_start, egui::Button::new("Démarrer serveur")).clicked() {
                ...
                if ui.add_enabled(can_trigger, egui::Button::new("⚠ Lancer OTA sur l'ESP")).clicked() {
                    // adapte au format de ta trame de commande existante
                    let cmd = vec![3]; // ex: opcode "start_ota"
            ...
            ui.label(format!("Serveur HTTP : {}", if status.running { "🟢 actif" } else { "🔴 arrêté" }));
            if let Some(ip) = &status.last_client_ip {
                ui.label(format!("Dernier client : {}", ip));
            }
            ui.label(format!("Bytes servis : {} KB", status.bytes_served / 1024));
            drop(status);

            ui.colored_label(egui::Color32::GOLD, "⚠️ Coupe toute commande moteur avant de lancer l'OTA");
